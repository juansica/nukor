...
function SettingsContent() {
  ...
  const fetchPlanStats = async () => {
    ...
    // Fetch integrations count from Ragie
    try {
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      { label: 'Entradas en biblioteca', value: entriesCount, sub: 'Ilimitado' },
                      { label: 'Conversaciones', value: conversationsCount, sub: 'Ilimitado' },
                      { label: 'Integraciones conectadas', value: integrationsCount, sub: null },
                    ].map(stat => (
                      <div key={stat.label} className="text-center p-4 bg-gray-50 rounded-lg">
                        <p className="text-2xl font-bold text-gray-950">{stat.value === null ? '—' : stat.value}</p>
                        <p className="text-xs text-gray-500 mt-1">{stat.label}</p>
                        {stat.sub && <p className="text-[11px] text-emerald-600 font-medium mt-0.5">{stat.sub}</p>}
                      </div>
                    ))}
                  </div>
                </div>
